...
class Notifiche5Giorni extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'reports:notifiche5-giorni';
    ...
    // Status per la notifica a 5 giorni
    private $notify_5_days_statuses = [
        'submitted',
        'user_replied',
        'reopened'
    ];
    ...
    public function handle()
    {
        ...
        $query = WbReport::whereIn('status', $this->notify_5_days_statuses)
    ...
    {
        if (!in_array($report->status, $this->notify_5_days_statuses)) {
            return;
        }

        $emailSent = $this->sendEmailStatus($investigators, $report, 'reminder_5_days', $daysPassed, $lastUpdate);
        
        if (!$emailSent) {
            Log::info("Email notification failed to send for Report #{$report->id} (5 days reminder)");
        }else{
            $this->info("Email notification sent to investigators for Report #{$report->id} (5 days reminder)");
        }
