...
class Notifiche90Giorni extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'reports:notifiche90-giorni';
    ...
    // Status per la notifica a 90 giorni
    private $notify_90_days_statuses = [
        'submitted',
        'read',
        'replied',
        'user_replied',
    ...
    {
        $fiveDaysAgo = Carbon::now()->subDays(90);
        $this->info('Five days ago: ' . $fiveDaysAgo);
        
        $query = WbReport::
            whereIn('status', $this->notify_90_days_statuses)
    ...
    {
        if (!in_array($report->status, $this->notify_90_days_statuses)) {
            return;
        }

        $emailSent = $this->sendEmailStatus($investigators, $report, 'reminder_90_days', $daysPassed, $lastUpdate);
        
        if (!$emailSent) {
            Log::info("Email notification failed to send for Report #{$report->id} (90 days reminder)");
        }else{
            $this->info("Email notification sent to investigators for Report #{$report->id} (90 days reminder)");
        }
